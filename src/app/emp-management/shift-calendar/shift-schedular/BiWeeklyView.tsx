...
import { Search,  CirclePlus, Edit, Info, Trash } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";

interface TwoWeeklyViewProps {
...
const TwoWeeklyView = ({
	...
	const renderShift = (
		...
		if (
			!isSameWeek(shift.start, currentDate, { weekStartsOn: 1 }) &&
			!isSameWeek(shift.start, addWeeks(currentDate, 1), {
				weekStartsOn: 1,
			})
		)
			return null;
		const startDay =
			(shift.start.getDay() || 7) +
			(isSameWeek(shift.start, addWeeks(currentDate, 1), {
				weekStartsOn: 1,
			})
				? 7
		...
		const endDay =
			(shift.end.getDay() || 7) +
			(isSameWeek(shift.end, addWeeks(currentDate, 1), {
				weekStartsOn: 1,
			})
				? 7
				: 0);
		const startOffset = startDay - 1;
		const duration = endDay - startDay + (endDay < startDay ? 14 : 0);
		return (
			<div
				key={shift.id}
				className="absolute bg-gray-100 border-l-4 border-l-black border border-gray-300 text-black text-[0.48rem] p-1 overflow-hidden cursor-pointer"
				style={{
					left: `${(startOffset / 14) * 100}%`,
					width: `${((duration || 1) / 14) * 100}%`,
					top: "2px",
					bottom: "2px",
				}}
				// onClick={(e) => {
				...
				// }}
			>
				<div className="font-semibold text-[0.50rem] leading-[10px]">
					{shift.title}
				</div>
			...
			<div className="flex">
				...
				{days.map((day) => (
					...
					>
						...
						<div>{format(day, "MMMM")}</div>
						<div className="text-[0.52rem]">2 People</div>
						<div className="text-[0.60rem]">
							9 h 15 m / <br /> £ 1,412.52
						</div>
					</div>
				))}
			</div>
