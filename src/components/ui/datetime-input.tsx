...
const DateTimeInput = React.forwardRef<HTMLInputElement, DateTimeInputProps>((options: DateTimeInputProps, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'flex h-8 items-center justify-between rounded-none border-b-2 border-b-gray-300 bg-background text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground  disabled:cursor-not-allowed disabled:opacity-50',
    ...
    >
      ...
      {!options.hideCalendarIcon && (
        <Button type='button' variant="ghost" className='hover:bg-transparent' size="icon" onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
